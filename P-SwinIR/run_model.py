...
parser.add_argument('--phy_scale', type = str, default= 'True')
...
########### Parameters ############
    BATCH_SIZE = 16 # please try 32 it should okay 
    NUM_WORKERS = 0 # on Windows, set this variable to 0
    scale_factor = args.scale_factor
    nb_epochs = 2000 # typically kernel will crush after 500 epochs
    ...
    model = SwinIR(upscale=scale_factor, in_chans=3, img_size=CROP_SIZE, window_size=8, img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffle', resi_conv='1conv').to(device)
    model = model.float()
    lossgen = LossGenerator(dx=data_dx,kernel_size= args.FD_kernel) 

    criterion1 = nn.L1Loss()
    criterion2 = nn.MSELoss()
    if args.loss_type =='L1':
        criterion_Data = nn.L1Loss()
    elif args.loss_tpye =='L2':
    ...
    for epoch in range(nb_epochs):
        ...
        for iteration, batch in enumerate(tqdm(trainloader)):
            ...
            phy_loss = criterion2(div,torch.zeros_like(div).cuda()) # DO NOT CHANGE THIS ONE. Phy loss has to be L2 norm 
        ...
        if hist_loss_val[-1] < best_loss_val:
            ...
            best_model = model
        savedpath = str("model_SwinIR_P_u." + str(args.scale_factor) + "_Loss_" + str(args.loss_type) + "_FD_kernel_" + str(args.FD_kernel) + "_gama_"
                   +str(args.phy_scale)
                   + "pt")
        torch.save({
                'epoch': epoch,
                ...
                'loss': loss,
                'PSNR': avg_psnr
                },savedpath ) # remember to change name for each experiment

    model = best_model


 torch.load("model_SwinIR.pt")
model.load_state_dict(checkpoint['model_state_dict'])
optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
...
print('relative error: %.6f' % np.sqrt(top_err/bot_err))