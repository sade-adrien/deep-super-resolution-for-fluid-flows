...
########### Parameters ############
    ...
    CROP_SIZE = 256
    gama = 0.1
    data = loadmat('nskt_T360_C3_S256.mat')
    trainset = DatasetFromTensor(torch.tensor(data['train']), scale_factor=scale_factor, with_bicubic_upsampling=False)
    ...
    for epoch in range(nb_epochs):
        ...
        for iteration, batch in enumerate(tqdm(trainloader)):
            ...
            data_loss = criterion2(out, target) # Experiment change to criterion 1
        ...
        epoch_loss = 0
        with torch.no_grad():
            for batch in testloader: # better be the val loader, need to modify datasets, but we are good for now.
                input, target = batch[0].to(device), batch[1].to(device)
                input, target = input.float(), target.float()
        ...
        torch.save({
                ...
                'loss': loss,
                'PSNR': avg_psnr
                }, "model_SwinIR_P_u8.pt") # remember to change name for each experiment
